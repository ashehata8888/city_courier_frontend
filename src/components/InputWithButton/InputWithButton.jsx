...
function InputWithButton({btnName , value , onChange ,trackingId ,parcel_status}) {
...
useEffect(()=>{
  ...
  const handlePatchData = async () => {
    const TRACKING_URL = btnName == "Picked Up" ? "http://localhost:7000/api/tracking/pickedup" : btnName == "Deliveried" ? "http://localhost:7000/api/tracking/deliveried" : null 
...
setIsSubmitted(true)
  ...
  return (
    <div>
      ...
      />
      <button disabled={isSubmitted}   className="btn btn-info rounde" onClick={handlePatchData}>{btnName}</button>
    </div>
  );
}
